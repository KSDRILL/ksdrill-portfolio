...
const navLinkClass = cn(
  "motion-safe-transition flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2.5 text-xs font-medium text-slate-400 hover:bg-white/[0.04] hover:text-slate-100 sm:px-2.5 sm:py-2 sm:text-sm",
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/[0.06] bg-slate-950/75 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-2 sm:py-3.5">
        <Link
          href="/"
          className="group flex min-h-[44px] min-w-0 shrink-0 items-center gap-2.5 sm:gap-3"
        >
          ...
          <div className="min-w-0">
            ...
            <div className="hidden font-mono text-[10px] uppercase tracking-[0.16em] text-slate-500 sm:block">
              {profile.titles.join(" · ")}
            </div>
          </div>
        </Link>
        <nav
          className="-mr-1 flex min-w-0 flex-1 items-center justify-end overflow-x-auto overscroll-x-contain py-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:mr-0 sm:justify-end [&::-webkit-scrollbar]:hidden"
          aria-label="Primary"
        >
