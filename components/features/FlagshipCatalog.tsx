    ...
    <>
      <div className="mb-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card className="border-slate-800/90 p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-500">CATALOG_SIZE</p>
          <p className="mt-2 text-2xl font-semibold tabular-nums text-slate-50">{systems.length}</p>
          <p className="mt-1 text-xs text-slate-500">flagship systems indexed</p>
        </Card>
        <Card className="border-slate-800/90 p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-500">ACTIVE_BUILD</p>
          <p className="mt-2 text-2xl font-semibold tabular-nums text-blue-300">{active}</p>
          <p className="mt-1 text-xs text-slate-500">in active development</p>
        </Card>
        <Card className="border-slate-800/90 p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-500">DEEP_EVIDENCE</p>
          <p className="mt-2 text-2xl font-semibold tabular-nums text-emerald-300/95">{withDiagram}</p>
          <p className="mt-1 text-xs text-slate-500">case studies with architecture diagrams</p>
        </Card>
        <Card className="border-slate-800/90 p-5">
          <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-slate-500">REPO_LINKS</p>
          <p className="mt-2 text-2xl font-semibold tabular-nums text-orange-300/90">{withLinks}</p>
          <p className="mt-1 text-xs text-slate-500">systems with GitHub or live links</p>
        </Card>
      </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="min-w-0 space-y-1">
                          ...
                          <h2 className="text-lg font-semibold text-slate-50">{system.name}</h2>
                        </div>
                      </div>
                      <ArrowUpRight className="h-5 w-5 shrink-0 text-slate-500 motion-safe:transition motion-safe:duration-180 group-hover:text-blue-400" aria-hidden />
                    </div>
                    <p className="mt-4 text-sm leading-relaxed text-slate-400">{system.shortDescription}</p>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {system.githubUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-lg border border-slate-700/80 bg-slate-900/60 px-3 py-1.5 font-mono text-[10px] text-slate-300 transition-colors hover:border-slate-500 hover:text-slate-100"
                    >
                  ...
                  {system.liveUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-1.5 font-mono text-[10px] text-blue-300 transition-colors hover:border-blue-400/50 hover:text-blue-200"
                    >
